...
int main()
{
	...
	int level_max = 12, level_threshold = 11, level_init = 1;
	...
	bool start_screen = true, testing = true;
	...
	std::string amazad_var = "Dark Hypersquare V0.23";
	...
	std::string bitmask_number_file[10];
	...
	scanner_sprite.setColor(sf::Color(full_intensity, full_intensity, full_intensity, max_transp));
	...
	level_sprite.setOrigin(sf::Vector2f(-4.5*squarep + 2*18, 5.5*squarep));
	...
	for (int a_sub = 0; a_sub < 10; a_sub++)
	{
		...
		number_sprite[a_sub].setOrigin(sf::Vector2f(-4.5*squarep - 2*18, 5.5*squarep + 1));
	...
	sf::Sprite dollar_sprite;
	dollar_sprite.setTexture(bitmask_dollar);
	
	dollar_sprite.setOrigin(sf::Vector2f(-4.5*squarep + 0*18, -1.5*squarep));
	
	dollar_sprite.setPosition(sf::Vector2f(0, -squarep));
	
	
	
	sf::Sprite dosh_sprite[10];
	
	for (int a_sub = 0; a_sub < 10; a_sub++)
	{
	
		dosh_sprite[a_sub].setTexture(bitmask_number[a_sub]);
	
		dosh_sprite[a_sub].setOrigin(sf::Vector2f(-4.5*squarep - 2*18, -1.5*squarep + 1));
	
		dosh_sprite[a_sub].setPosition(sf::Vector2f(0, -squarep));
	...
	while (window.isOpen())
    {
		...
		for (int b_sub = 0; b_sub <= 255; b_sub++)
		{
		...
		for (int b_sub = outro; b_sub >= 0; b_sub--)
		{
		...
		for (int level = level_init; level <= level_max; level++)
		{
			...
			if (testing && (level >= 2) && (level <= level_threshold))
			{
			...
			if ((level == 2) || (level > level_threshold))
			{
			...
			if ((level == 3) || (level > level_threshold))
			{
			...
			if ((level >= 8) && (level <= level_threshold))
			{
			...
			for (int a_sub = 0; a_sub < 10; a_sub++)
			{
				...
				dosh_sprite[a_sub].setPosition(sf::Vector2f(0, -squarep));
				dosh_sprite[a_sub].setRotation(0);
			...
			while (!level_change)
			{
				...
				if (!view_glitch && !exhale)
				{
					...
					while (crunchy_number > 0)
					{
					
						window.draw(number_sprite[crunchy_number % 10]);
						...
						for (int a_sub = 0; a_sub < 10; a_sub++)
						{
							...
							number_sprite[a_sub].move(sf::Vector2f(18*dir_left[0], 18*dir_left[1]));
					...
					for (int a_sub = 0; a_sub < 10; a_sub++)
					{
						...
						number_sprite[a_sub].move(sf::Vector2f(crunched*18*dir_right[0], crunched*18*dir_right[1]));
					...
					if (crunchy_number == 0)
					{
						...
						window.draw(dosh_sprite[crunchy_number]);
					...
					while (crunchy_number > 0)
					{
						...
						window.draw(dosh_sprite[crunchy_number % 10]);
						...
						for (int a_sub = 0; a_sub < 10; a_sub++)
						{
							...
							dosh_sprite[a_sub].move(sf::Vector2f(18*dir_left[0], 18*dir_left[1]));
					...
					dollar_sprite.move(sf::Vector2f(crunched*18*dir_left[0], crunched*18*dir_left[1]));
					
					for (int a_sub = 0; a_sub < 10; a_sub++)
					{
							
						dosh_sprite[a_sub].move(sf::Vector2f(crunched*18*dir_right[0], crunched*18*dir_right[1]));
							
					}
								
					window.draw(dollar_sprite);		
					
					dollar_sprite.move(sf::Vector2f(crunched*18*dir_right[0], crunched*18*dir_right[1]));
					...
					if (background_blink_on)
					{
						...
						if (background_blink < max_transp)
						{
							...
							background_blink = background_blink + 2;
				...
				{	
						...
						!action)
					{
						...
						dollar_sprite.move(sf::Vector2f(-level_side*squarep, 0));

						for (int a_sub = 0; a_sub < 10; a_sub++)
						{
								
							number_sprite[a_sub].move(sf::Vector2f(-level_side*squarep, 0));
							dosh_sprite[a_sub].move(sf::Vector2f(-level_side*squarep, 0));
						...
						!action)
					{
						...
						dollar_sprite.move(sf::Vector2f(0, -level_side*squarep));

						for (int a_sub = 0; a_sub < 10; a_sub++)
						{
								
							number_sprite[a_sub].move(sf::Vector2f(0, -level_side*squarep));
							dosh_sprite[a_sub].move(sf::Vector2f(0, -level_side*squarep));
					...
					for (int actions = 1; actions <= transitions; actions++)
					{
						...
						if (moving)
						{
							...
							if (actions <= 21)
							{
								...
								dollar_sprite.move(sf::Vector2f(final_move[0], final_move[1]));

								for (int a_sub = 0; a_sub < 10; a_sub++)
								{
									
									number_sprite[a_sub].move(sf::Vector2f(final_move[0], final_move[1]));
									dosh_sprite[a_sub].move(sf::Vector2f(final_move[0], final_move[1]));
						...
						if (turn_right || turn_left)
						{
							...
							if (actions <= 21)
							{
								...
								dollar_sprite.rotate(final_rot);

								for (int a_sub = 0; a_sub < 10; a_sub++)
								{
									
									number_sprite[a_sub].rotate(final_rot);
									dosh_sprite[a_sub].rotate(final_rot);
