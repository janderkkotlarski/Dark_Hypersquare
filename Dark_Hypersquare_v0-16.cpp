...
int main()
{
	...
	bool dark_backed = false, first_dark_back = false;
	...
	std::string amazad_var = "Dark Hypersquare V0.16";
	
	std::string scanner_img = "A-M4Z2-D_Scanner.png";
	
	std::string arrow_img = "Arrow_Small.png";
	
	std::string compass_back_img = "Compass_Back_Icon.png";
	
	std::string compass_img = "Compass_Icon.png";
	...
	scanner_sprite.setColor(sf::Color(full_intensity, full_intensity, full_intensity, 128));
	...
	double compass_x_a = 0, compass_y_a = 5*squarep;
	
	double compass_x_b = 0, compass_y_b = 5*squarep;
	
	double compass_x_c = 0, compass_y_c = -squarep;
	
	double compass_x_d = 0, compass_y_d = -squarep;
	
	
	
	double qompass_x, qompass_y;
	
	double kompass_x, kompass_y;
	
	double kompazz_x, kompazz_y;
	
	double delta_kompass_x, delta_kompass_y;
	
	double move_kompass_x, move_kompass_y, move_kompazz_x, move_kompazz_y;
	...
	compass_back_sprite.setOrigin(sf::Vector2f(compass_x_a + 0.5*squarep, compass_y_a + 0.5*squarep));
	
	compass_back_sprite.setPosition(sf::Vector2f(compass_x_c, compass_y_c));
	...
	compass_sprite.setOrigin(sf::Vector2f(compass_x_a + 0.5*squarep, compass_y_a + 0.5*squarep));
	
	compass_sprite.setPosition(sf::Vector2f(compass_x_c, compass_y_c));
	...
	arrow_sprite.setOrigin(sf::Vector2f(compass_x_b + 0.5*squarep, compass_y_b + 0.5*squarep));
	
	arrow_sprite.setPosition(sf::Vector2f(compass_x_d, compass_y_d));
	...
	level_sprite.setOrigin(sf::Vector2f(-3.5*squarep, 5.5*squarep));
	...
	for (int a_sub = 1; a_sub <= level_max; a_sub++)
	{
		...
		number_sprite[a_sub].setOrigin(sf::Vector2f(-4.5*squarep, 5.5*squarep));
	...
	infobox = Square_Draw(infobox, color_black, 0*max_transp, 0, -squarep, 0, 0);
	...
	while (window.isOpen())
    {
		...
		for (int level = 1; level <= level_max; level++)
		{
			...
			if (level == 1)
			{
				...
				square_matrix[max_level][2 + max_level] = 3;
				
				square_matrix[max_level][3 + max_level] = 3;
				
				square_matrix[max_level][4 + max_level] = 3;
			...
			compass_x_a = 0;
			compass_y_a = 5*squarep;

			compass_x_b = 0;
			compass_y_b = 5*squarep;
			...
			while (!level_change)
			{
				...
				if (dark_setback)
				{
					...
					compass_x_b = compass_x_b + delta_kompass_x;
					compass_y_b = compass_y_b + delta_kompass_y;
				...
				if (!view_glitch && !exhale)
				{
					...
					compass_back_sprite.setColor(sf::Color(255 - colours[0]/2, 255 - colours[1]/2, 255 - colours[2]/2, max_transp));
				...
				{	
						...
						!action)
					{
						...
						compass_x_a = compass_x_a - level_side*squarep;
						
						compass_x_b = compass_x_b - level_side*squarep;
						
						compass_x_c = compass_x_c - level_side*squarep;
						
						compass_x_d = compass_x_d - level_side*squarep;
						...
						!action)
					{
						...
						compass_y_a = compass_y_a - level_side*squarep;
						
						compass_y_b = compass_y_b - level_side*squarep;
						
						compass_y_c = compass_y_c - level_side*squarep;
						
						compass_y_d = compass_y_d - level_side*squarep;
					...
					if (action && moving)
					{
						...
						if (square_matrix[local_x + max_level][local_y + max_level] == 3)
						{
							...
							delta_kompass_x = 0 - compass_x_d - 1*dir_direct[0]*squarep;
									
							delta_kompass_y = -squarep - compass_y_d - 1*dir_direct[1]*squarep;
							
							if (!first_dark_back)
							{
								
								delta_kompass_x = delta_kompass_x - 1*dir_direct[0]*squarep;
								
								delta_kompass_y = delta_kompass_y - 1*dir_direct[1]*squarep;
								
								first_dark_back = true;
					...
					for (int actions = 1; actions <= transitions; actions++)
					{
						...
						if (moving)
						{
							...
							if (actions <= 21)
							{
								...
								compass_x_a = compass_x_a + final_move[0];
						
								compass_y_a = compass_y_a + final_move[1];
								
								compass_x_b = compass_x_b + final_move[0];
						
								compass_y_b = compass_y_b + final_move[1];
								
								compass_x_c = compass_x_c + final_move[0];
								
								compass_y_c = compass_y_c + final_move[1];
								
								compass_x_d = compass_x_d + final_move[0];
								
								compass_y_d = compass_y_d + final_move[1];
						...
						if (turn_right || turn_left)
						{
							...
							qompass_x = compass_x_b;

							qompass_y = compass_y_b - squarep;
	
							if (dark_backed && true)
							{
								
								qompass_y = compass_y_b - 2*squarep;
								
							}

						
							kompass_x = compass_x_d;
				
							kompass_y = compass_y_d;
							...
							if (actions <= 21)
							{
								...
								// compass_back_sprite.setOrigin(sf::Vector2f(compass_x + 0.5*squarep, compass_y + 0.5*squarep));								
								// compass_back_sprite.rotate(-final_rot);
																
								// compass_back_sprite.setOrigin(sf::Vector2f(compass_x_c + 0.5*squarep, compass_y_c + 0.5*squarep));
								
								
								
								compass_sprite.rotate(final_rot);			
													
								// compass_sprite.setOrigin(sf::Vector2f(compass_x + 0.5*squarep, compass_y + 0.5*squarep));										
								// compass_sprite.rotate(-final_rot);
																
								// compass_sprite.setOrigin(sf::Vector2f(compass_x_c + 0.5*squarep, compass_y_c + 0.5*squarep));
								

								
								// arrow_sprite.setPosition(sf::Vector2f(compass_x_c, compass_y_c));
								
								// arrow_sprite.rotate(final_rot);			
													
								// arrow_sprite.setPosition(sf::Vector2f(compass_x, compass_y));	
								// arrow_sprite.setOrigin(sf::Vector2f(compass_x + 0.5*squarep, compass_y + 0.5*squarep));																			
								// arrow_sprite.rotate(-final_rot);
								
								// window.draw(arrow_sprite);
																
								// arrow_sprite.setPosition(sf::Vector2f(compass_x_c, compass_y_c));
								// arrow_sprite.setOrigin(sf::Vector2f(compass_x_c + 0.5*squarep, compass_y_c + 0.5*squarep));
								
								
								
								compass_x_d = (kompass_x - qompass_x)*cos(pi*1.0*final_rot/180) - (kompass_y - qompass_y)*sin(pi*1.0*final_rot/180) + qompass_x;
								
								compass_y_d = (kompass_x - qompass_x)*sin(pi*1.0*final_rot/180) + (kompass_y - qompass_y)*cos(pi*1.0*final_rot/180) + qompass_y;
						...
						for (int a_sub = -max_view; a_sub <= max_view; a_sub++)
						{
							...
							for (int b_sub = -max_view; b_sub <= max_view; b_sub++)
							{
									...
									!level_change && !level_back && !(dark_setback || dark_flicker))
								{
									...
									sukuwarii = Square_Draw(sukuwarii, karasu, 128, pos_x, pos_y, paruto*squarrel, paruto*squarrel);
