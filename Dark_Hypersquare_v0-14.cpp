...
int main()
{
	...
	std::string amazad_var = "Dark Hypersquare V0.14";
	...
	scanner_sprite.setColor(sf::Color(255, 255, 255, 128));
	...
	double compass_x_c = 0, compass_y_c = 0;
	
	double compass_x = 0, compass_y = -squarep;
	
	double kompass_x = 0, kompass_y = 0;
	
	double compazz_x, compazz_y;
	...
	sf::RectangleShape infobox(sf::Vector2f(2*half_wind, half_wind - 1.5*squarep));
	
	Color_Picker(0, color_black, exit_colors);
	
	infobox.setOrigin(sf::Vector2f(half_wind, -1.5*squarep));
		...
		for (int level = 1; level <= level_max; level++)
		{
			...
			while (!level_change)
			{
				...
				if (dark_setback)
				{
					...
					compass_back_sprite.setOrigin(sf::Vector2f(compass_x_c + 0.5*squarep, compass_y_c + 0.5*squarep));
					compass_back_sprite.setPosition(sf::Vector2f(compass_x, compass_y));			
				
					
					compass_sprite.setOrigin(sf::Vector2f(compass_x_c + 0.5*squarep, compass_y_c + 0.5*squarep));
					compass_sprite.setPosition(sf::Vector2f(compass_x, compass_y));			
					...
					infobox.setPosition(sf::Vector2f(0, 0));	
				...
				{	
						...
						!action)
					{
						...
						compass_x = compass_x - level_side*squarep;
						...
						kompass_x = kompass_x - level_side*squarep;
						...
						!action)
					{
						...
						compass_y = compass_y - level_side*squarep;
						...
						kompass_y = kompass_y - level_side*squarep;
					...
					if ((((square_matrix[local_x + dir_direct[0] + max_level][local_y + dir_direct[1] + max_level] != 1) ||
						...
						((square_matrix[-size_level + max_level][local_y + max_level] != 1) && (local_x == size_level) && (local_x + dir_direct[0] > size_level)) ||
						((square_matrix[local_x + max_level][-size_level + max_level] != 1) && (local_y == size_level) && (local_y + dir_direct[1] > size_level))) && true) ||
						((square_matrix[local_x + dir_direct[0] + max_level][local_y + dir_direct[1] + max_level] != 10) ||
						...
						((square_matrix[-size_level + max_level][local_y + max_level] != 10) && (local_x == size_level) && (local_x + dir_direct[0] > size_level)) ||
						((square_matrix[local_x + max_level][-size_level + max_level] != 10) && (local_y == size_level) && (local_y + dir_direct[1] > size_level))) && true)) &&
						false)
					{
						...
						if ((local_x > size_level) && (square_matrix[-size_level + max_level][local_y + max_level] != 1) &&
							(square_matrix[-size_level + max_level][local_y + max_level] != 10))
						{
						...
						if ((local_y > size_level) && (square_matrix[local_x + max_level][-size_level + max_level] != 1) &&
							(square_matrix[local_x + max_level][-size_level + max_level] != 1))
						{
					...
					for (int actions = 1; actions <= transitions; actions++)
					{
						...
						if (moving)
						{
							...
							if (actions <= 21)
							{
								...
								compass_x = compass_x + final_move[0];
						
								compass_y = compass_y + final_move[1];
								...
								kompass_x = kompass_x + final_move[0];
								
								kompass_y = kompass_y + final_move[1];								
