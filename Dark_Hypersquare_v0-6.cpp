...
int main()
{
	...
	int delaz = 3, squarep = 50, window_x = 12*squarep, window_y = 12*squarep, squarrel = 20;
	...
	int color_black[3] = {0, 0, 0}, key_colour[3];
	...
	std::string amazad_var = "Dark Hypersquare V0.6";
	...
	// std::string bitmask_key_a_file = "Key_A.png";
	// std::string bitmask_key_left_turn_file = "Key_Turn_Left_Icon.png";
	
	// std::string bitmask_key_w_file = "Key_W.png";
	// std::string bitmask_exchange_file = "Exchange_Icon.png";
	
	// std::string bitmask_x_file = "Key_X.png";
	// std::string bitmask_build_file = "Build_Icon.png";
	
	// std::string bitmask_r_file = "Key_R.png";
	// std::string bitmask_reset_file = "Reset_Icon.png";
	
	// std::string bitmask_r_file = "Key_V.png";
	// std::string bitmask_reset_file = "Timeshift_Icon.png";
	...
	while (window.isOpen())
    {
		...
		for (int level = 1; level <= level_max; level++)
		{
			...
			while (!level_change)
			{
				...
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::W) && !exhale)
				{
				...
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::X) && !inhale && !exhale)
				{
				...
				if ((inhale || building) && exchange && !exhale && !action && !(level_reset || level_recet))
				{
				...
				if (action)
				{
					...
					for (int actions = 1; actions <= transitions; actions++)
					{
						...
						if ((inhale || exhale || building) && !level_change && !(dark_setback || dark_flicker))
						{
