...
int main()
{
	...
	int level_max = 10, level_threshold = 50;
	...
	int color_black[3] = {0, 0, 0}, key_colour[3], background_colour[3], shadow_colours[3];
	...
	std::string amazad_var = "Dark Hypersquare V0.21";
					...
					for (int a_sub = -max_view; a_sub <= max_view; a_sub++)
					{
						...
						for (int b_sub = -max_view; b_sub <= max_view; b_sub++)
						{
							...
							{			
								...
								bitshadow_sprite.setColor(sf::Color(background_blink, background_blink, background_blink, round(dark_transp*max_transp)));
								...
								if (square_matrix[pot_x + max_level][pot_y + max_level] == 2)
								{
									...
									bitmask_sprite.setColor(sf::Color(key_colour[0], key_colour[1], key_colour[2], round(dark_transp*max_transp)));
				...
				if (action)
				{
					...
					for (int actions = 1; actions <= transitions; actions++)
					{
						...
						for (int a_sub = -max_view; a_sub <= max_view; a_sub++)
						{
							...
							for (int b_sub = -max_view; b_sub <= max_view; b_sub++)
							{
									...
									(square_matrix[pot_x + max_level][pot_y + max_level] != 3))
								{
									...
									bitshadow_sprite.setColor(sf::Color(background_blink, background_blink, background_blink, round(dark_transp*1.0*max_transp)));
									...
									if (square_matrix[pot_x + max_level][pot_y + max_level] == 2)
									{
										...
										bitmask_sprite.setColor(sf::Color(key_colour[0], key_colour[1], key_colour[2], round(dark_transp*1.0*max_transp)));
									...
									!level_change && !level_back && !(dark_setback || dark_flicker))
								{
									...
									{	
										...
										if (pacman == 2)
										{
											...
											sukuwarii = Square_Draw(sukuwarii, key_colour, bat_transp, pos_x, pos_y, paruto*squarrel, paruto*squarrel);
						...
						if ((inhale || exhale || building) && !level_change && !level_back && !(dark_setback || dark_flicker))
						{
							...
							squaree = Square_Draw(squaree, color_black, max_transp, scan_pos_x + dir_up[0]*squarep, scan_pos_y + dir_up[1]*squarep, squarrel, squarrel);
