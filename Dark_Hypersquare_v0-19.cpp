...
int main()
{
	...
	std::string amazad_var = "Dark Hypersquare V0.19";
	...
	std::string bitmask_img = "Bit_Mask_40c.png";
		...
		for (int level = 1; level <= level_max; level++)
		{
			...
			if (level == 8)
			{
				...
				for (int a_sub = -4; a_sub <= size_level; a_sub++)
				{
					...
					square_matrix[max_level - size_level][max_level + a_sub] = 0;
				...
				for (int a_sub = 0; a_sub <= size_level; a_sub++)
				{
					...
					square_matrix[max_level + a_sub][max_level + size_level] = 0;
					
				}
				
				square_matrix[max_level - size_level][max_level] = 10;
			...
			if (level == 4)
			{
				...
				square_matrix[max_level][max_level - 2] = 4;
			...
			while (!level_change)
			{
					...
					for (int a_sub = -max_view; a_sub <= max_view; a_sub++)
					{
						...
						for (int b_sub = -max_view; b_sub <= max_view; b_sub++)
						{
							...
							if (square_matrix[pot_x + max_level][pot_y + max_level] == 2)
							{
								...
								squaree = Square_Draw(squaree, key_colour, round(dark_transp*square_transp), pos_x, pos_y, squarrel, squarrel);
				...
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::X) && build && !inhale && !exhale && !action)
				{
				...
				{	
					...
					if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up) && up_movement)
					{
					...
					if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down) && down_movement)
					{
					...
					if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right) && right_movement)
					{
					...
					if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left) && left_movement)
					{
